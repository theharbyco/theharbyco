// 🔥 FIREBASE CONFIG (already pasted by you)

// BOOK DATA (ADMIN UPLOAD SIMULATION)
const books = [
...
function renderCart(){
 ...
 cart.forEach(c=>{html+=`<p>${c.title} - ₹${c.price}</p>`;total+=c.price})
...
// LOGIN METHODS (Firebase connected)
function emailLogin(){alert("Email Login Connected")}
function googleLogin(){alert("Google Login Connected")}
function phoneLogin(){alert("OTP Login Connected")}
